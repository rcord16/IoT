...
set_ccode() {
	...
	killall -USR1 ustatsd  # pause ustatsd

	/bin/radarprescan clear
	/usr/etc/init.d/plugin stop fastpath
	/usr/etc/init.d/plugin stop airview
	...
	/usr/etc/init.d/plugin start airview
	/usr/etc/init.d/plugin start fastpath
	killall ubntspecd
	/usr/bin/garp

	killall -USR2 ustatsd  # resume ustatsd
}
